...
league_mask = df_both_seasons['Div'].str.contains('SP') #for english premierleague
#df_both_seasons[(df_both_seasons['HomeTeam']=='Logrones')]
df_both_seasons = df_both_seasons[ time_mask & league_mask]
teamMask = ((df_both_seasons['HomeTeam']=='Logrones') &(df_both_seasons['AwayTeam']=='Sabadell')& (df_both_seasons['Date']=='2020-11-15'))
index= 6 
df_both_seasons.loc[teamMask,'FTR'] = 'A'
df_both_seasons.loc[teamMask,'AGoals'] = 3
df_both_seasons.loc[teamMask,'AGoalDiff'] = 3
df_both_seasons.loc[teamMask,'HGoalDiff'] = -3
#print(df_both_seasons.loc[teamMask])
...
for col in average_columns:
    for side in sides:
        df_both_seasons[side+col+'_Avg'] = (df_both_seasons['Avg{}{}_{}'.format(side,col,averageOfWeeks[0])]*weigth[0]+df_both_seasons['Avg{}{}_{}'.format(side,col,averageOfWeeks[1])]*weigth[1]+df_both_seasons['Avg{}{}_{}'.format(side,col,averageOfWeeks[2])]*weigth[2]+df_both_seasons['Avg{}{}_{}'.format(side,col,averageOfWeeks[3])]*weigth[3])/sum(weigth)
...
h_cols = df_result_h.columns.tolist()
a_cols = df_result_a.columns.tolist()
all_cols =  h_cols+a_cols
print(all_cols)
for col in all_cols:
...
#7/0
algoName= 'predictions_ByLastSeason_SpainNoRatio3Col'
#for lim in [3,5,8,13]:
import matplotlib.pyplot as plt
import seaborn as sns
def showPlot(x,y):
...
diff_columns = df_both_seasons[(['Div','Date','HomeTeam','AwayTeam','pre_HGoals','pre_AGoals','HGoals','AGoals',
                                'AvgH','AvgA']+[col+'Diff' for col in all_cols])]
